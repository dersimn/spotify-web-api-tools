class SpotifyWebApiTools {
    ...
    async getAtLeast(initialPromise, pagingObjectSelector, count) {
        const temporary = [];
        for await (const snippet of this.processNextGenerator(
            initialPromise,
            pagingObjectSelector
        )) {
            temporary.push(...snippet);
            if (count && temporary.length > count) {
                return temporary;
            }
        }
    ...
    async getAllUserPlaylists(userId, options) {
        return this.getAll(this.webApi.getUserPlaylists(userId, options), ['body']);
    }
...
export default SpotifyWebApiTools;
